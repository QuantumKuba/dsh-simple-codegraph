...
import { Config, findCodeGraphRoot, hasCodeGraphIndex } from './types.js'
...
/** Active agent session handle tracking per-agent MCP disposal. */
...
export function apply(ctx: Context, config: Config): void {
  const activeSessions = new Map<Agent, ActiveSession>()

  /**
   * Mount an isolated CodeGraph MCP client for one live Agent.
   */
  async function install(agent: Agent): Promise<void> {
    ...
    if (!cwd || typeof cwd !== 'string') {
      ...
      ctx.logger.debug?.(
        'dsh-simple-codegraph: agent "%s" has no usable session cwd; skipping CodeGraph mount',
        agent.id,
      )
      return
    }
    ...
    if (globalTool !== undefined) {
      ctx.logger.warn(
        'dsh-simple-codegraph: existing global CodeGraph MCP integration detected (mcp__codegraph__codegraph_explore). ' +
    ...
    const ready = (async () => {
      try {
        ...
        ctx.logger.warn(
          `dsh-simple-codegraph: failed to start CodeGraph MCP for agent "${agent.id}": ${String(error)}`,
        )
        throw error
      }
    })()

    const handle: ActiveSession = { dispose, ready }
    ...
    if (config.routingHint && hasCodeGraphIndex(cwd)) {
      try {
        agent.ctx?.inject?.(['systemPrompt'], (promptScope: Context) => {
          promptScope.systemPrompt.section({
            name: 'codegraph:routing',
            order: 2000,
            text: () =>
              "For repository discovery, CodeGraph's `codegraph_explore` is exposed in DSH as `mcp__codegraph__codegraph_explore`. " +
